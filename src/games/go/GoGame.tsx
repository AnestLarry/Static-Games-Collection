...
const GoGame: React.FC<GoGameProps> = ({ navigate }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 bg-gray-100 min-h-screen">
      <h1 className="text-4xl font-extrabold text-gray-800 mb-6 drop-shadow-md">Go Game</h1>
      <div className="flex flex-wrap justify-center sm:justify-between w-full max-w-md mb-4 text-center sm:text-left">
        <p className="text-lg mb-2 sm:mb-0 sm:mr-4">Current Player: <span className="font-semibold capitalize">{gameState.currentPlayer}</span></p>
        <p className="text-lg mb-2 sm:mb-0 sm:mr-4">Black Captured: <span className="font-semibold">{gameState.capturedWhite}</span></p>
        <p className="text-lg">White Captured: <span className="font-semibold">{gameState.capturedBlack}</span></p>
      </div>
